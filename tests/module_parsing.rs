...
fn test_hello_ptx_module() {
    let source = fs::read_to_string("tests/sample/hello.ptx")
        .expect("Failed to read hello.ptx");
    ...
    assert!(!module.directives.is_empty(), "Module should have directives");
    eprintln!("hello.ptx: Successfully parsed {} directives", module.directives.len());
}
...
fn test_matrix_ptx_module() {
    ...
    // Verify we parsed directives
    assert!(!module.directives.is_empty(), "Module should have directives");
    eprintln!("matrix file: Successfully parsed {} directives", module.directives.len());
}
