...
fn parse_statements(source: &str) -> Vec<FunctionStatement> {
    ...
    while !stream.is_at_end() {
        let (stmt, _) =
            FunctionStatement::parse()(&mut stream).expect("statement parse should succeed");
...
fn parse_label_and_instruction(source: &str) -> (String, Instruction) {
    ...
    let label = match label_stmt {
        FunctionStatement::Label { label, .. } => label.val,
...
fn test_instruction_with_label_and_predicate() {
    util::run_with_large_stack(|| {
        let (label, inst) =
            parse_label_and_instruction("my_label: @%p1 setp.eq.s32 %p0, %r1, %r2;");
...
fn test_complex_label_names() {
    util::run_with_large_stack(|| {
        ...
        match &statements[0] {
            FunctionStatement::Label { label, .. } => assert_eq!(label.val, "loop_start_123"),
            other => panic!("expected label statement, got {:?}", other),
        }
    });
}
