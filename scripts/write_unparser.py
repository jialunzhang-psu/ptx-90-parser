...
def main():
    ...
    for i, instruction_name in enumerate(instructions_to_process):
        ...
        prompt = f"""Now you are writing a part of a PTX code unparser. Read `src/type/instruction/{instruction_name}.rs` and `src/parser/mod.rs` and `src/type/common.rs`. In `src/parser/instruction/{instruction_name}.rs`, implement the `PtxUnparser` trait (in `src/unparser/mod.rs`) for the instruction. 
