    ...
    fn tree_display(&self, f: &mut TreeFormatter, source: &str) -> std::fmt::Result {
        match self {
            ModuleVariableDirective::Tex { directive, span } => {
                f.root(&format!("ModuleVariableDirective::Tex [{}]", f.format_raw(*span, source)))?;
                f.field_with_child(true, "directive", directive, source)
            }
            ModuleVariableDirective::Shared { directive, span } => {
                f.root(&format!("ModuleVariableDirective::Shared [{}]", f.format_raw(*span, source)))?;
                f.field_with_child(true, "directive", directive, source)
            }
            ModuleVariableDirective::Global { directive, span } => {
                f.root(&format!("ModuleVariableDirective::Global [{}]", f.format_raw(*span, source)))?;
                f.field_with_child(true, "directive", directive, source)
            }
            ModuleVariableDirective::Const { directive, span } => {
                f.root(&format!("ModuleVariableDirective::Const [{}]", f.format_raw(*span, source)))?;
                f.field_with_child(true, "directive", directive, source)
            }
        }
    }
...
impl TreeDisplay for VariableDirective {
    fn tree_display(&self, f: &mut TreeFormatter, source: &str) -> std::fmt::Result {
        f.root(&format!("VariableDirective [{}]", f.format_raw(self.span, source)))?;
    ...
    fn tree_display(&self, f: &mut TreeFormatter, source: &str) -> std::fmt::Result {
        match self {
            VariableModifier::Vector { value, span } => {
                f.root(&format!("VariableModifier::Vector [{}]", f.format_raw(*span, source)))?;
                f.field(true, "value", &value.to_string())
            }
            VariableModifier::Alignment { value, span } => {
                f.root(&format!("VariableModifier::Alignment [{}]", f.format_raw(*span, source)))?;
                f.field(true, "value", &value.to_string())
            }
            VariableModifier::Ptr { span } => {
                f.root(&format!("VariableModifier::Ptr [{}]", f.format_raw(*span, source)))
            }
        }
    }
}
    ...
    fn tree_display(&self, f: &mut TreeFormatter, source: &str) -> std::fmt::Result {
        match self {
            ParameterDirective::Register { ty, name, span } => {
                f.root(&format!("ParameterDirective::Register [{}]", f.format_raw(*span, source)))?;
                f.field_with_child(false, "ty", ty, source)?;
                f.field(true, "name", &name.val)
            }
            ParameterDirective::Parameter { align, ptr, space, ty, name, array, span } => {
                f.root(&format!("ParameterDirective::Parameter [{}]", f.format_raw(*span, source)))?;
                match align {
...
impl TreeDisplay for ParamStateSpace {
    fn tree_display(&self, f: &mut TreeFormatter, source: &str) -> std::fmt::Result {
        ...
        f.root(&format!("ParamStateSpace::{} [{}]", variant, f.format_raw(*span, source)))
    }
}
    ...
    fn tree_display(&self, f: &mut TreeFormatter, source: &str) -> std::fmt::Result {
        match self {
            InitializerValue::NumericLiteral { value, span } => {
                f.root(&format!("InitializerValue::NumericLiteral [{}]", f.format_raw(*span, source)))?;
                f.field(true, "value", &format!("{:?}", value))
            }
            InitializerValue::FunctionSymbol { name, span } => {
                f.root(&format!("InitializerValue::FunctionSymbol [{}]", f.format_raw(*span, source)))?;
                f.field(true, "name", &name.val)
            }
            InitializerValue::StringLiteral { value, span } => {
                f.root(&format!("InitializerValue::StringLiteral [{}]", f.format_raw(*span, source)))?;
                f.field(true, "value", &format!("\"{}\"", value))
            }
        }
    }
    ...
    fn tree_display(&self, f: &mut TreeFormatter, source: &str) -> std::fmt::Result {
        match self {
            GlobalInitializer::Scalar { value, span } => {
                f.root(&format!("GlobalInitializer::Scalar [{}]", f.format_raw(*span, source)))?;
                f.field_with_child(true, "value", value, source)
            }
            GlobalInitializer::Aggregate { values, span } => {
                f.root(&format!("GlobalInitializer::Aggregate [{}] ({} values)",
                    f.format_raw(*span, source), values.len()))
            }
        }
    }
}
