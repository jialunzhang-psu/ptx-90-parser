...
pub mod section_0 {
    ...
    impl PtxParser for MbarrierCompleteTxSemScopeSpaceB64 {
        fn parse() -> impl Fn(&mut PtxTokenStream) -> Result<(Self, Span), PtxParseError> {
            try_map(
                seq_n!(
                    string_p("mbarrier"),
                    string_p(".complete_tx"),
                    optional(Sem::parse()),
                ...
                |(_, complete_tx, sem, scope, space, b64, addr, _, txcount, _), span| {
                    ok!(MbarrierCompleteTxSemScopeSpaceB64 {
