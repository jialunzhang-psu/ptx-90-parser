use crate::{
    ...
    r#type::{
        ...
        instruction::barrier::{
            BarArrive, BarReductionLogical, BarReductionPopc, BarSync, Barrier, BarrierArrive,
...
fn parse_barrier_instruction(stream: &mut PtxTokenStream) -> Result<Barrier, PtxParseError> {
...
fn parse_bar_instruction(stream: &mut PtxTokenStream) -> Result<Barrier, PtxParseError> {
    let scope = Scope::parse(stream)?;
    let (modifier, span) = stream.expect_directive()?;
    match modifier.as_str() {
...
fn parse_barrier_sync(stream: &mut PtxTokenStream, scope: Scope) -> Result<Barrier, PtxParseError> {
...
fn parse_barrier_arrive(
    stream: &mut PtxTokenStream,
    scope: Scope,
) -> Result<Barrier, PtxParseError> {
    let aligned = consume_directive_if(stream, "aligned");
    let barrier = Operand::parse(stream)?;
    ...
    Ok(Barrier::Arrive(BarrierArrive {
        scope,
        aligned,
        barrier,
        expected_count,
...
fn parse_barrier_reduction_popc(
    ...
    Ok(Barrier::ReductionPopc(BarrierReductionPopc {
        scope,
        aligned,
        data_type,
        destination,
...
fn parse_barrier_reduction_logical(
    ...
    Ok(Barrier::ReductionLogical(BarrierReductionLogical {
        scope,
        aligned,
        destination,
        barrier,
...
fn parse_bar_sync(stream: &mut PtxTokenStream, scope: Scope) -> Result<Barrier, PtxParseError> {
    let barrier = Operand::parse(stream)?;
    let expected_count = if stream
        .consume_if(|token| matches!(token, PtxToken::Comma))
        .is_some()
...
fn parse_bar_arrive(stream: &mut PtxTokenStream, scope: Scope) -> Result<Barrier, PtxParseError> {
    let barrier = Operand::parse(stream)?;
    stream.expect(&PtxToken::Comma)?;
    let expected_count = Operand::parse(stream)?;
    stream.expect(&PtxToken::Semicolon)?;

    Ok(Barrier::BarArrive(BarArrive {
        scope,
        barrier,
        expected_count,
    }))
...
fn parse_bar_reduction_popc(
    ...
    Ok(Barrier::BarReductionPopc(BarReductionPopc {
        scope,
        data_type,
        destination,
        barrier,
...
fn parse_bar_reduction_logical(
    ...
    Ok(Barrier::BarReductionLogical(BarReductionLogical {
        scope,
        destination,
        barrier,
        expected_count,
...
fn parse_optional_expected_count_and_predicate(
    ...
    if next_is_predicate_input(stream)? {
