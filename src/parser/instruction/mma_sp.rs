...
pub mod section_0 {
    ...
    impl PtxParser for Spvariant {
        fn parse() -> impl Fn(&mut PtxTokenStream) -> Result<(Self, Span), PtxParseError> {
            alt!(
                map(string_p(".sp::ordered_metadata"), |_, _span| {
                    Spvariant::SpOrderedMetadata
                }),
                map(string_p(".sp"), |_, _span| Spvariant::Sp)
            )
        }
    }
                ...
                |(
                 ...
                 span| {
                    ok!(MmaSpOrderedMetadataSyncAlignedM16n8k64RowColKindDtypeF8f6f4typeF8f6f4typeCtype {
                        sp_ordered_metadata = sp_ordered_metadata,
                        sync = sync,
                        aligned = aligned,
                        m16n8k64 = m16n8k64,
                        row = row,
...
pub mod section_2 {
    ...
    impl PtxParser for Spvariant {
        fn parse() -> impl Fn(&mut PtxTokenStream) -> Result<(Self, Span), PtxParseError> {
            alt!(map(string_p(".sp::ordered_metadata"), |_, _span| {
                Spvariant::SpOrderedMetadata
            }))
        }
    }
    ...
    impl PtxParser for MmaSpvariantSyncAlignedM16n8k64RowColKindBlockScaleScaleVecSizeF32F8f6f4typeF8f6f4typeF32Stype {
        fn parse() -> impl Fn(&mut PtxTokenStream) -> Result<(Self, Span), PtxParseError> {
            try_map(
                ...
                |(_, spvariant, sync, aligned, m16n8k64, row, col, kind, block_scale, scale_vec_size, f32, f8f6f4type, f8f6f4type1, f322, stype, d, _, a, _, b, _, c, _, e, _, f, _, scale_a_data, _, byte_id_a, _, scale_b_data, _, byte_id_b, _), span| {
