...
impl PtxParser for Rcp {
    fn parse(stream: &mut PtxTokenStream) -> Result<Self, PtxParseError> {
        let (opcode, opcode_span) = stream.expect_identifier()?;
        if opcode != "rcp" {
            return Err(unexpected_value(opcode_span, &["rcp"], opcode));
        }
...
fn parse_approx(stream: &mut PtxTokenStream) -> Result<Rcp, PtxParseError> {
    let flush_to_zero =
...
fn parse_rnd(stream: &mut PtxTokenStream) -> Result<Rcp, PtxParseError> {
