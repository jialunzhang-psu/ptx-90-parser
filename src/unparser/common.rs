...
use crate::{
    ...
    r#type::common::{
        ...
        Operand, PredicateRegister, RegisterOperand, Sign, SpecialRegister, TexType,
        VariableSymbol,
    },
};
...
pub(crate) fn push_double_colon(tokens: &mut Vec<PtxToken>) {
...
pub(crate) fn push_flush_to_zero(tokens: &mut Vec<PtxToken>, enabled: bool) {
    if enabled {
...
impl PtxUnparser for RegisterOperand {
    fn unparse_tokens(&self, tokens: &mut Vec<PtxToken>) {
        match self {
            RegisterOperand::Single(name) => push_register(tokens, name),
            RegisterOperand::Vector2(registers) => {
                tokens.push(PtxToken::LBrace);
                push_register(tokens, &registers[0]);
                tokens.push(PtxToken::Comma);
                push_register(tokens, &registers[1]);
                tokens.push(PtxToken::RBrace);
            }
            RegisterOperand::Vector4(registers) => {
                tokens.push(PtxToken::LBrace);
