...
#[derive(Debug, Clone)]
pub struct AnalyzedSection(pub Vec<AnalyzedInstruction>);
...
pub enum AnalyzedOperandElement {
    ...
    /// [xdim, ydim, zdim, ...]
    SquareGroup(Vec<(IdentifierToken, RustName)>),
    /// (param-list)
    ParamList(RustName),
    ...
    /// First field is the base name, second is the options
    Choice {
        base: (IdentifierToken, RustName),
        options: Vec<IdentifierToken>,
    },
}
...
impl Analyzer {
                ...
                *count += 1;
                ...
                for operand in &mut instr.operands {
                    ...
                    if let Some((_op, rust_name)) = &mut operand.operator {
                        let base = format!("{}_op", self.compute_operand_element_base_name(&operand.operand));
    ...
    fn analyze_operand(&self, operand: &Operand) -> AnalyzedOperand {
        AnalyzedOperand {
            operator: operand.operator.as_ref().map(|x| (x.clone(), "".to_string())),
            operand: self.analyze_operand_element(&operand.operand),
            modifier: operand.modifier.as_ref().map(|m| (self.analyze_modifier(m), "".to_string())),
        }
    }
        ...
        match element {
            Item(ident) => {
                ...
                if let Some(choices) = self.parameters.get(ident) {
                    // Extract identifier tokens from the choices
                    let options: Vec<IdentifierToken> = choices
                        .iter()
                        .filter_map(|choice| match choice {
                            Modifier::Atom(id) => Some(id.clone()),
                            Modifier::ImmediateNumber(num) => Some(num.clone()),
                            _ => None,
                        })
                        .collect();
            ...
            PipeChoice(pair) => AnalyzedOperandElement::PipeChoice(((pair.0.clone(), "".to_string()), (pair.1.clone(), "".to_string()))),
            PipeOptionalChoice(pair) => AnalyzedOperandElement::PipeOptionalChoice(((pair.0.clone(), "".to_string()), (pair.1.clone(), "".to_string()))),
            CurlyGroup(operands) => {
                // CurlyGroup can contain simple items or nested structures
                let items: Vec<(IdentifierToken, RustName)> = operands
                    .iter()
                    .filter_map(|op| match &op.operand {
                        Item(ident) => Some((ident.clone(), "".to_string())),
                        Optional(ident) => Some((ident.clone(), "".to_string())),
                        _ => None,
                    })
                    .collect();
            ...
            SquareGroup(operands) => {
                ...
                // For now, we'll recursively analyze each operand element
                // and extract identifier names where possible
                let items: Vec<(IdentifierToken, RustName)> = operands
                    .iter()
                    .filter_map(|op| match &op.operand {
                        Item(ident) => Some((ident.clone(), "".to_string())),
                        Optional(ident) => Some((ident.clone(), "".to_string())),
                        _ => {
                            // For complex nested elements, we could recursively analyze
                            // but for now skip them to avoid the panic
                            None
                        }
                    })
                    .collect();
            ...
            ImmediateNumber(num) => AnalyzedOperandElement::ImmediateNumber((num.clone(), "".to_string())),
        }
    }
    ...
    fn analyze_modifier(&self, modifier: &Modifier) -> AnalyzedModifier {
        match modifier {
            ...
            Modifier::ImmediateNumber(num) => {
                // Immediate numbers are just passed through as atoms
                AnalyzedModifier::Atom((num.clone(), "".to_string()))
            }
                    ...
                    items
                        ...
                        .map(|ident| {
                            ...
                            if let Some(choices) = self.parameters.get(ident) {
                                ...
                                (AnalyzedModifier::Atom((ident.clone(), "".to_string())), "".to_string())
                            }
                        })
                        .collect(),
                )
...
impl Analyzer {
    fn compute_modifier_rust_names(&self, modifier: &mut AnalyzedModifier, field_names: &mut HashMap<String, usize>) {
        match modifier {
    ...
    fn compute_operand_element_rust_names(&self, element: &mut AnalyzedOperandElement, field_names: &mut HashMap<String, usize>) {
        match element {
            ...
            | AnalyzedOperandElement::PipeOptionalChoice(((first, first_name), (second, second_name))) => {
            ...
            AnalyzedOperandElement::Choice { base, options: _ } => {
                let (base_ident, base_rust_name) = base;
                // Enum names should be PascalCase
    ...
    fn compute_operand_element_base_name(&self, element: &AnalyzedOperandElement) -> String {
        match element {
            ...
            AnalyzedOperandElement::CurlyGroup(items) | AnalyzedOperandElement::SquareGroup(items) => {
                items.iter().map(|(ident, _)| ident.as_str()).collect::<Vec<_>>().join("_")
            }
